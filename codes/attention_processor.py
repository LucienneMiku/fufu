...
class Attention(nn.Module):
    ...
    def forward(
        ...
        **cross_attention_kwargs,
        ...
        attn_parameters = set(processor_signature.parameters.keys())
        accepts_var_kwargs = any(
            p.kind == inspect.Parameter.VAR_KEYWORD for p in processor_signature.parameters.values()
        )
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if encoder_hidden_states is not None:
            ...
            dynamic_mask_controller = kwargs.get("dynamic_mask_controller", None)
            if dynamic_mask_controller is not None and kwargs.get("capture_cross_attn", False):
                with torch.no_grad():
                    dynamic_mask_controller.consume_query_key(
                        query=query,
                        key=key,
        ...
        # PyTorch SDPA requires additive bias masks to match query dtype.
