...
class LamicProcessor:
    ...
    def preprocess_prompt(self, inputs, pipe, ref_num, processed_inputs, padding_prompts, max_sequence_length):
        ...
        clip_prompt = str(inputs.get("prompt", "") or "").strip()
        if not clip_prompt:
            clip_prompt = str(inputs.get("CEI", "") or "").strip()
        # Load SAD
        ...
        processed_inputs["clip_prompt"] = clip_prompt if clip_prompt else total_prompts
    ...
    def preprocess_region(self, inputs, pipe, ref_num, processed_inputs, height, width, resize_output_size, save_bbox_masks=True):
        ...
        processed_inputs["latent_height"] = latent_height
        processed_inputs["latent_width"] = latent_width
        processed_inputs["packed_latent_height"] = latent_height // 2
        processed_inputs["packed_latent_width"] = latent_width // 2
    ...
    def _get_attention_mask(self, index_mask, processed_inputs, stage: Optional[int] = None):
        ...
        target_device = qk_mask.device

        # Keep all index tensors on one device to avoid meshgrid device mismatch.
        for key, token_idx in processed_inputs["token_indices"].items():
            if token_idx.device != target_device:
                processed_inputs["token_indices"][key] = token_idx.to(target_device)

        for key, token_idx in processed_inputs["mitigated_indices"].items():
            if token_idx.device != target_device:
                processed_inputs["mitigated_indices"][key] = token_idx.to(target_device)
